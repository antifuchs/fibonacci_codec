...
// use bit_vec::BitVec;
use fibonacci_codec::{DecodeError, EncodeSlice, decode_u64, decode_u8};
...
fn test_roundtrip_u64() {
    ...
    let decoded: Vec<u64> = decode_u64(encoded).map(|x| x.unwrap()).collect();
...
fn test_roundtrip_u8() {
    ...
    let encoded = input.fib_encode().unwrap();
    let decoded: Vec<u8> = decode_u8(encoded).map(|x| x.unwrap()).collect();
    assert_eq!(input, decoded);
}
...
fn test_overflow_from_fib_elt() {
    ...
    let encoded = input.fib_encode().unwrap();
    let decoded: Vec<Result<u8, DecodeError>> = decode_u8(encoded).collect();
    assert_eq!(
